...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE (dar.activity LIKE '".$searchingg."' OR dar.plan LIKE '".$searchingg."' OR dar.result LIKE '".$searchingg."') AND (dar.status='late1' OR dar.status='late' OR dar.status='late2' OR dar.status='over') ORDER BY dar.tanggaldar DESC;";
$querydar = $conn->query($infodar);
$arrayrdar = mysqli_fetch_all($querydar, MYSQLI_ASSOC);
mysqli_close($conn);
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE (dar.status='late1' OR dar.status='late' OR dar.status='late2' OR dar.status='over') ORDER BY dar.tanggaldar DESC;";
$querydar = $conn->query($infodar);
$arrayrdar = mysqli_fetch_all($querydar, MYSQLI_ASSOC);
mysqli_close($conn);
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.nik, masteruser.departemen, dar.jam, dar.tanggaldar, dar.status, dar.sudahbaca, dar.tanggal, dar.urid, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE tanggal BETWEEN CAST('".$tglstart."' AS DATE) AND CAST('".$tglend."' AS DATE) AND (dar.status='late1' OR dar.status='late' OR dar.status='late2' OR dar.status='over') AND lokasikerja = 'IGI Purwosari' ORDER BY dar.tanggaldar DESC";
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.nik, masteruser.departemen, dar.jam, dar.tanggaldar, dar.status, dar.sudahbaca, dar.tanggal, dar.urid, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE tanggal BETWEEN CAST('".$tglstart."' AS DATE) AND CAST('".$tglend."' AS DATE) AND (dar.status='late1' OR dar.status='late' OR dar.status='late2' OR dar.status='over') AND lokasikerja = 'IGI Bambe' ORDER BY dar.tanggaldar DESC";
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.nik, masteruser.departemen, dar.jam, dar.tanggaldar, dar.status, dar.sudahbaca, dar.tanggal, dar.urid, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE tanggal BETWEEN CAST('".$tglstart."' AS DATE) AND CAST('".$tglend."' AS DATE) AND (dar.status='late1' OR dar.status='late' OR dar.status='late2' OR dar.status='over') AND lokasikerja = 'sda' ORDER BY dar.tanggaldar DESC";
                       ...
                       <!-- DataTales Example -->
                          <div class="card shadow mb-4">
                            <div class="card-header py-3">
                             <h4 class="card-title m-0 text-uppercase font-weight-bold">Late Status</h4>
                            </div>
                                <div class="card-body">
                                 <h6>Period Filter:</h6>
                                 <div class="row">
                ...
                $echosts = '<span class="badge badge-late">Late2</span>';  
                  ...
                  <tr> 
                    ...
                    <td><a style="font-weight:<?php echo $sama ?>;" id="linkk" href="status.php?idnya=<?php echo $arrayrdar[$x]["id"] ?>&nama=<?php echo $arrayrdar[$x]["nama"] ?>"><?php echo $arrayrdar[$x]["nama"] ?></td></a>
                     <td><a style="font-weight:<?php echo $sama ?>;" id="linkk" href="status.php?idnya=<?php echo $arrayrdar[$x]["id"] ?>&nama=<?php echo $arrayrdar[$x]["nama"] ?>"><?php echo $arrayrdar[$x]["nik"] ?></td></a>
                    <td><a style="font-weight:<?php echo $sama ?>;" id="linkk" href="status.php?idnya=<?php echo $arrayrdar[$x]["id"] ?>&nama=<?php echo $arrayrdar[$x]["nama"] ?>"><?php 
...
$tgldar = $daydar .'-'. $mondar .'-'. $yerdar;
                    ...
                    echo $tgldar; ?></td></a>
                    <td ><a style="font-weight:<?php echo $sama ?>;" id="linkk" href="status.php?idnya=<?php echo $arrayrdar[$x]["nodar"] ?>"><?php
...
echo $darsend;                  
                   ...
                   <td ><a style="font-weight:<?php echo $sama ?>;" id="linkk" href="status.php?idnya=<?php echo $arrayrdar[$x]["nodar"] ?>"><?php echo $echosts ?></td></a>
          
                   
                   
                    <td style="display:none" ><a id="linkk" style="font-weight:<?php echo $sama ?>;" href="status.php?idnya=<?php echo $arrayrdar[$x]["nodar"] ?>"><?php echo $arrayrdar[$x]["tag"] ?></td></a>
