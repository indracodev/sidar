...
$jmlstringresult = strlen($result);
$jmlstringactivity = strlen($activity);
...
$ambilnoabsensi = "SELECT noabsensi FROM absenluarkota WHERE tglsubmit = '" .$tgldarnya . "' AND iduser =  '" .$IUser . "' ORDER BY idabsen DESC LIMIT 2;";
...
$ambiltgdar = "SELECT tanggaldar FROM dar WHERE iduser = '" .$IUser . "' ORDER BY `urid` DESC;";
$querytgdar =$conn->query($ambiltgdar);
$arraytgdar = mysqli_fetch_array($querytgdar, MYSQLI_ASSOC);
$tgdaruser = $arraytgdar['tanggaldar'];
...
if($jmlstringresult >= 12 && $jmlstringactivity >= 12){    
...
echo '<script>
	alert("isi dar anda masih ada yang kosong ");
	alert("silakan lengkapi isi dar anda kembali ");
...
echo '<script>
	alert("Maaf, Tidak Bisa Kirim Dar 2 Kali");
	window.location.assign("dar.php");
</script>' ;
