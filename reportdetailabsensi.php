...
$ambildistributor = "SELECT * FROM datadist WHERE iduser = '" .$idsr . "' AND status = 'aktif';";

 $querydistributor =$conn->query($ambildistributor);
 if($querydistributor->num_rows){
   session_start();
   $rowdist = mysqli_fetch_all($querydistributor, MYSQLI_ASSOC);
  
  // $namadist = $rowdist['namadist'];
  // $alamatdist = $rowdist['alamatdist'];
  // $sts = $rowdist['status'];
  // $longitudedist = $rowdist['longitude'];
  // $latitudedist = $rowdist['latitude'];

 }
 else{
    // header("location: http://www.icg.id/beta/dar/login");
   echo " Data distributor belum ada ";
 }
...
<body>
	...
	<div class="wrapper">
        ...
        <div class="capture-area">
                <div id="content" class="container">
                    <header>
                        ...
                        <div class="kanan text-left mt-2">
                          ...
                          <div class="tanggal-report" style="height: 20px;font-size:15px;">
                                   ...
                                   <label class="font-weight-bold" style="width:270px;text-align: left;">: <?php echo $area; ?></label>
                          ...
                          <div class="tanggal-report" style="height: 20px;font-size:15px;">
                                   ...
                                   <label class="font-weight-bold" style="width:190px;text-align: left;">: <?php echo ucwords($status); ?></label>
                    ...
                    <div style="WHITE-SPACE: NORMAL;padding-top: 3px;" class="ql-editor activityToday mb-2 notranslate">
  ...
  <div class="row">
    ...
    <iframe src="https://maps.google.com/maps?q=<?php echo $latitude ?>, <?php echo $longitude ?>&z=15&output=embed" width="455" height="500" frameborder="0" style="border:0"></iframe>
    </div>
    <div class="col">
            <h5 class="font-weight-bold">Distributor</h5>
      <div id='map'></div>
    </div>
  </div>
    ...
    <script>
	mapboxgl.accessToken = 'pk.eyJ1Ijoic2dpd2ViIiwiYSI6ImNrc3E1bDFpazA5cnIyd252amJ6dmV6YzgifQ.eGVUcbmYW0T0vZ3rfZkEFw';
const map = new mapboxgl.Map({
container: 'map',
style: 'mapbox://styles/sgiweb/cktjhwdpb76m217p7ra13mcmm',
...
const namae = new mapboxgl.Popup({ offset: 25 }).setText(
'<?php echo $nama;?>'
); 


// Create a default Marker, colored black, rotated 45 degrees.
const markerorang = new mapboxgl.Marker({ color: 'red', rotation: 45 })
.setLngLat([<?php echo $longitude;?>, <?php echo $latitude;?>])
.setPopup(namae)
.addTo(map);

<?php for($x = 0; $x < sizeof($rowdist); $x++){ ?>

const popup<?php echo $x;?> = new mapboxgl.Popup({ offset: 25 }).setText(
'<?php echo $rowdist[$x]['namadist'];?>'
); 
// Create a default Marker and add it to the map.
const marker<?php echo $x;?> = new mapboxgl.Marker()
.setLngLat([<?php echo $rowdist[$x]['longitude'];?>, <?php echo $rowdist[$x]['latitude'];?>])
.setPopup(popup<?php echo $x;?>)
.addTo(map);
