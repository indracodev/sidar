...
if($do == "category"){
  if(!isset($_POST["hash"])){
    ...
    if($hash == ""){
      ...
      if($countHashData != 1){
        ...
        $pid = 0;
        $getCategory = $conn->query("SELECT * FROM artikel_kategori WHERE artikel_kategori_status = 'Aktif' AND artikel_kategori_id != 1 ");
  ...
  if($hash == ""){
    ...
    if($countHashData != 1){
      ...
      if($catID == ""){
        $getList = $conn->query("SELECT * FROM artikel WHERE artikel_static = 'Blogpost' AND artikel_kategori_id != 1 AND artikel_status = 'Terbit' ORDER BY artikel_time DESC ");
      } else {
        $getList = $conn->query("SELECT * FROM artikel WHERE artikel_static = 'Blogpost' AND artikel_kategori_id != 1 AND artikel_status = 'Terbit' AND artikel_kategori_id = '".$catID."' ORDER BY artikel_time DESC ");
      }
      $pid = 0;
