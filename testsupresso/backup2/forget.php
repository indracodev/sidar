...
if($do == "mailcode"){
      ...
      if($countHashData != 1){
        ...
        if($countIsset >= 3){
          ...
          if($countUserData != 1){
            ...
            //Generating Body Mail
            $mailBodyHTML = '<html lang="en" style="padding: 0; margin: 0; background-color: #ffffff; width: 100%!important">
            <head>
              <meta charset="utf-8">
            ...
            <body style="padding: 0; margin: 0; background-color: #ffffff; width: 100%!important">
              <table width="100%" cellpadding="0" cellspacing="0" border="0" align="center" style="width: 100%!important"><tbody>
                <tr style="display: none; max-height: 0; overflow: hidden"><td valign="top" align="left">
                  ...
                  <editable name="preheader">Supresso - Forget Password</editable>
                  &nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;
  ...
  if(!isset($_POST["hash"])){
    ...
    if($hash == ""){
      ...
      if($countHashData != 1){
        ...
        //Get Request
        $getRequest = $conn->query("SELECT * FROM apiforget WHERE apihash_hash = '".$hash."' AND apiforget_email = '".$mail."' ");
        ...
        if($countRequest == 0){
          ...
          $dateNow = date("Y-m-d");
          $getRequest1 = $conn->query("SELECT * FROM apiforget WHERE apihash_hash = '".$hash."' AND apiforget_email = '".$mail."' AND apiforget_date = '".$dateNow."' AND apiforget_status = 'active' ");
  ...
  if(!isset($_POST["hash"])){
    ...
    if($hash == ""){
      ...
      //Get request data
      $dateNow = date("Y-m-d");
      $getRequest = $conn->query("SELECT * FROM apiforget WHERE apihash_hash = '".$hash."' AND apiforget_email = '".$email."' AND apiforget_status = 'permited' AND apiforget_date = '".$dateNow."' ");
      $countRequest = $getRequest->num_rows;
      if($countRequest != 1){
