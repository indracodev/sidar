...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE (dar.activity LIKE '".$searchingg."' OR dar.plan LIKE '".$searchingg."' OR dar.result LIKE '".$searchingg."') AND (dar.status='late1' OR dar.status='late2' OR dar.status='over') ORDER BY dar.tanggaldar DESC;";
$querydar = $conn->query($infodar);
$arrayrdar = mysqli_fetch_all($querydar, MYSQLI_ASSOC);
mysqli_close($conn);
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE (dar.status='late1' OR dar.status='late2' OR dar.status='over') ORDER BY dar.tanggaldar DESC;";
$querydar = $conn->query($infodar);
$arrayrdar = mysqli_fetch_all($querydar, MYSQLI_ASSOC);
mysqli_close($conn);
...
$infodar = "SELECT masteruser.id, masteruser.nama, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE (dar.status='late1' OR dar.status='late2' OR dar.status='over') ORDER BY dar.tanggaldar DESC, dar.jam DESC;";
                       ...
                       <!-- DataTales Example -->
                          <div class="card shadow mb-4">
                            <div class="card-header py-3">
                             <h4 class="card-title m-0 text-uppercase font-weight-bold">Harvest</h4>
                            </div>
                                <div class="card-body">
                                 <h6>Period Filter:</h6>
                                 <div class="row">
