        ...
        if ($userResult->num_rows > 0) {
            ...
            if ($sql->execute() === TRUE) {
                ...
                if ($sql->execute() === TRUE) {
                    ...
                    $sql->bind_param('s', $postSchedule); $sql->execute();
                    $schedule = $sql->get_result();
                    if ($schedule->num_rows > 0) {
                        $sql = $conn->prepare("DELETE FROM tb_task_list WHERE scheduleId = ?");
                        $sql->bind_param('s', $postSchedule);
                        if ($sql->execute() === TRUE) {
                            $output->status = 'success';
                            echo(json_encode($output));
            ...
            while ($rowProgress = $progress->fetch_assoc()) {
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($diff->format("%R") == '-') {
                            $sql = $conn->prepare("SELECT tb_task.taskId, taskName FROM tb_task, tb_task_list, tb_schedule WHERE tb_schedule.scheduleId = tb_task_list.scheduleId AND tb_task.taskId = tb_task_list.taskId AND tb_schedule.scheduleId = ?");
                            $sql->bind_param('s', $row['scheduleId']); $sql->execute();
                            ...
                            switch ($row['activityStatus']) {
                                case '1':
                                    $sql = $conn->prepare("SELECT tb_task.taskId, taskName, taskStatus FROM tb_task, tb_task_list, tb_schedule WHERE tb_schedule.scheduleId = tb_task_list.scheduleId AND tb_task.taskId = tb_task_list.taskId AND tb_schedule.scheduleId = ?");
                                    $sql->bind_param('s', $row['scheduleId']); $sql->execute();
                                    $taskResult = $sql->get_result();
                                    if ($taskResult->num_rows > 0) {
                                        while ($rowTask = $taskResult->fetch_assoc()) {
