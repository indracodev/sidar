        ...
        if ($resultCheck->num_rows > 0) {
            ...
            if ($activityResult->num_rows > 0) {
                ...
                while ($activityRow = $activityResult->fetch_assoc()) {
                    ...
                    $sql = $conn->prepare("SELECT scheduleId, personName, tb_report.activityId, GROUP_CONCAT(checkpointName separator ' to ') as checkpointName, GROUP_CONCAT(reportLatitude) as latitude, GROUP_CONCAT(reportLongitude) as longitude, activityStart, activityEnd, reportNote
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $sql = $conn->prepare("SELECT taskName, taskStatus FROM tb_task_list, tb_task WHERE tb_task_list.taskId = tb_task.taskId AND scheduleId = ?");
