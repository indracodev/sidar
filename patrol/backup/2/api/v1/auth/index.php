...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($postName) && !empty($postName) && $postName != 'undefined' &&
        ...
        $sql->bind_param("ss", $postName, $password); $sql->execute();
            ...
            while ($row = $result->fetch_assoc()) {
                if ($row["userLevel"] > 0) {
                    if ($password == $row["userPassword"] && $username == $row["userName"]) {
                        ...
                        $sql = $conn->prepare($query); $sql->bind_param("ss", $hash, $username);
                        if ($sql->execute() === TRUE) {
                            if (!empty($action)) {
                                switch ($postDevice) {
                                    case 'web':
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($postHash) && !empty($postHash) && $postHash != 'undefined' &&
        ...
        $sql = $conn->prepare($query); $sql->bind_param("s", $postHash); $sql->execute();
        $result = $sql->get_result();
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                switch ($postDevice) {
