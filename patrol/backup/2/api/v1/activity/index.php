        ...
        switch ($activityAction) {
            ...
            case 'end':
                ...
                if ($getCheckpoint->num_rows > 0) {
                    ...
                    $sql = $conn->prepare("SELECT DISTINCT phaseId FROM tb_schedule, tb_activity WHERE tb_activity.activityId = tb_schedule.activityId AND tb_schedule.scheduleDate = ? AND tb_schedule.scheduleStart <= ? AND tb_schedule.scheduleEnd >= ? AND tb_activity.activityStatus = '0' ORDER BY tb_schedule.uid ASC LIMIT 1");
                    $sql->bind_param('sss', date('y-m-d'), date('H:i:s'), date('H:i:s')); $sql->execute();
                    $getPhase = $sql->get_result();
                    if ($getPhase->num_rows > 0) {
                        $phaseId;
