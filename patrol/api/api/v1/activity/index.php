    ...
    if (isset($mappingTag) && !empty($mappingTag) && $mappingTag != 'undefined' &&
                        ...
                        case 'Z':
                            ...
                            if ($getPhase->num_rows > 0) {
                                ...
                                if ($getSchedule->num_rows > 0) {
                                    ...
                                    $sql = $conn->prepare("SELECT tb_person.personId FROM tb_person, tb_person_mapping, tb_schedule, tb_activity
                                        WHERE tb_schedule.mappingId = tb_person_mapping.mappingId AND tb_schedule.personId = tb_person.personId AND tb_schedule.scheduleId = tb_activity.scheduleId AND mappingTag = ? AND activityStatus = '0'
                                        ORDER BY scheduleDate ASC, scheduleStart ASC LIMIT 1"); $sql->bind_param('s', $mappingTag);
                                    $sql->execute();
        ...
        if ($resultCheck->num_rows > 0) {
            while ($rowCheck = $resultCheck->fetch_assoc()) {
                ...
                $sql = $conn->prepare("INSERT INTO tb_activity (activityId, personId, activityStatus) VALUES (?, ?, '0')"); $sql->bind_param("is", $activityId, $mappingTag);
                if ($sql->execute() === TRUE) {
                    $output->status = 'success';
                    $output->activity = $activityId;
                    echo(json_encode($output));
