...
$infodar = "SELECT masteruser.nama, masteruser.nik, masteruser.departemen, dar.status, dar.sudahbaca, dar.tanggaldar, dar.jam, dar.tanggal, dar.nodar, dar.tag FROM dar INNER JOIN masteruser ON masteruser.id=dar.iduser WHERE dar.iduser='".$iduser."' AND (dar.status='late' OR dar.status='over') ORDER BY dar.tanggaldar DESC;";
$querydar = $conn->query($infodar);
$arrayrdar = mysqli_fetch_all($querydar, MYSQLI_ASSOC);
}
